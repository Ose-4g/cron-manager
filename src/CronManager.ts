...
export class CronManager{
    ...
    register(Class:Function, instance:any):void{
        if(!this.instanceMap.has(Class.name)) throw new Error(`class ${Class.name} has not been decorated with @cronGroup`)
    ...
    private init()
    {
        ...
        groups.forEach((value)=>{
            ...
            handlers.forEach((handler)=>{
                ...
                if(!this.instanceMap.has(className)) throw new Error(`class ${className} was not decorateed with cronGroup tag`);
